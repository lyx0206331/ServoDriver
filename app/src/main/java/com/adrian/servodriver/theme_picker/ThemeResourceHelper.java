...
public class ThemeResourceHelper {
    ...
    private static ThemeResourceHelper mInstance;  //  静态实例对象
     ...
     *
     * @param context
     * @return ThemeResourceHelper
     */
    public static ThemeResourceHelper getInstance(@NonNull Context context) {
        if (mInstance == null) {
            synchronized (ThemeResourceHelper.class) {
                if (mInstance == null) mInstance = new ThemeResourceHelper(context);
            }
        } else {
